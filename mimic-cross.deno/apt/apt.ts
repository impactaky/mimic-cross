...
import { deployPackageCommands, deployCrossTool } from "./helper.ts";
import { supportedPackagesPromise } from "./load.ts";
import { dynamicImport } from "import/mod.ts";
  ...
  for (const p of filteredPackages) {
    ...
    switch (packageInfo.postInstall) {
      ...
      case "skip":
        ...
        break;
      default: {
        const modulePath = `file://${config.mimicCrossRoot}/apt/packages/modules/${packageInfo.postInstall}.ts`;
        let module = undefined;
        try {
          module = await dynamicImport(modulePath);
        } catch (e) {
          logger.error(`(deployPackages) Fail import ${modulePath} : ${e}`);
          throw e;
        }
